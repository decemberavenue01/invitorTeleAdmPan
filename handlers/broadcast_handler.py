...
async def perform_broadcast(msg_or_call, state: FSMContext):
    ...
    for user_id in users:
        try:
            if 'photo' in data:
                await msg_or_call.bot.send_photo(chat_id=user_id, photo=data['photo'], caption=data['text'], reply_markup=keyboard)
            elif 'video_note' in data:
                await msg_or_call.bot.send_video_note(chat_id=user_id, video_note=data['video_note'], reply_markup=keyboard)
            else:
                await msg_or_call.bot.send_message(chat_id=user_id, text=data['text'], reply_markup=keyboard)
            count += 1
